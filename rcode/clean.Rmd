...
Add skew var kurt and mean (dhist)

PROBLEMS: 
lags are not grouped by rn (dhist)
weird duplicate donations (dhist)
...
dhist10s <- f %>% dplyr::filter(lagtype == "ten" | lagtype == "unknown") %>% select(donation_id,urlsolid, lag1,lag2,lag3,lag4,lag5, lag6, lag7, lag8, lag9, lag10)

variance10 <- apply(dhist10s[, 3:12], 1, var)
skew10 <- apply(dhist10s[, 3:12], 1, skewness) #then just mutate that column otherwise 
kurtosis10 <- apply(dhist10s[, 3:12], 1, kurtosis)
mean10 <- apply(dhist10s[, 3:12], 1, mean)
#mean10 <- gapply(dhist10s[, 3:1                                                                                                                                                                                                                ccc2], 1, mean) 

dhist10s$kurtosis <- kurtosis10
dhist10s$variance <- variance10
dhist10s$skewness <- skew10
dhist10s$mean <- mean10
dhist10s <- dhist10s %>% select(donation_id,kurtosis, variance, skewness, mean)
g <- left_join(f, dhist10s, by ="donation_id")


dhist5s <- f %>% dplyr::filter(lagtype == "5") %>% select(donation_id,urlsolid, lag1,lag2,lag3,lag4,lag5)

variance5 <- apply(dhist[,3:7], 1, var)
skew5 <- apply(dhist[,3:7], 1, skewness) 
kurtosis5 <- apply(dhist[,3:7], 1, kurtosis)
mean5 <- apply(dhist[,3:7], 1, mean)

dhist5s$kurt <- kurtosis5
dhist5s$var <- variance5
dhist5s$skew <- skew5
dhist5s$avg <- mean5

dhist5s <- dhist5s %>% select(donation_id, kurt, var, skew, avg)
g <- left_join(f, dhist5s, by ="donation_id")

dhist <- dhist %>% dplyr::filter(urlsolid == "#NAME?") #need to add this at some pt

head(dhist,20)
nrow(dhist)
write.csv(dhist,'dhist1to4.csv')
```
