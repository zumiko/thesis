...
TODO: 
Add criteria for incomplete 1000 + (dhist) (think that this is done)
Add index for which number each donations is 
...
maxedout <- function(dhist){#gets the number of each donor, gets camps maxed out
  ...
  dhist <- dhist %>% group_by(urlsolid) %>%
    mutate(numdonor = seq_len(length.out = countd)) #add the 
  dhist
}
...
```{r}
#just as a check everything works
a<- prepdhist(dhistall)
b<- addtimevars(a)
c<- leaddate(b)
...
m <- dhist %>% group_by(urlsolid) %>% summarise(eh = n())
m <- dhist %>% select(-eh) %>% mutate(numcamp = seq_len(nrow(m)))
set.seed(9)
split = sample(c(TRUE, FALSE), nrow(m), replace=TRUE, prob=c(0.5, 0.5))
m <- cbind(m, split)
dhist <- left_join(dhist, m, by = "urlsolid")
