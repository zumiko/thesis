...
#paths
```{r}
path2 <- "/Users/CJ/Desktop/thesisdata/sample2" #path to timehist data 
#pathzip <- "/Users/CJ/Desktop/thesisdata/sample2" #path to zipdata 
pathzip <- "/Users/CJ/Desktop/thesisdata/sample2/aaa/campattributes.csv" #path to campattributes
```

```{r}
campattributes <- read.csv(pathzip, row.names=1)
```



```{r}
dfhist <-lapply_read_csv_bind_rows(path2, pattern = "*.csv")
```
...
```{r}
count(dfhist, urlsolid)
```

```{r}
cname <- dfhist %>% filter(urlsolid ==  "28b3kc")
cname
```

```{r}
#head(campatt)
cname2 <- campatt %>% filter(urlname == "28b3kc")
cname2
```
