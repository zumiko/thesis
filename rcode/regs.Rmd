...
dfc10 <- dfc %>% filter(lagtype == "ten")
dfc5 <- dfc %>% filter(lagtype == "five")
it <- lm(amount ~ histavg + seedvalue, data = dhist)
...
```{r}
minlm <- lm(amount ~ d2, data = dhistsize)
summary(minlm)
medianlm <- lm(amount ~ d3, data = dhistsize)
summary(medianlm)
maxlm <- lm(amount ~ d5, data = dhistsize)
summary(maxlm)
alllm <- lm(amount ~ d1 + d2 + I(d2^2) + d3 + d4 + d5, data = dhistsize)
summary(alllm)
allinterlm <- lm(amount ~ d1*d2 + d1*d3 + d1*d4 + d1*d5 + d2*d3 + d2*d4 + d2*d5 + d3*d4 + d3*d5 + d4*d5, data = dhistsize)
summary(allinterlm)
...
```{r}
skewvar <- lm(amount ~ histavg+ skewness + variance + d1 + seedvalue, data = dhistsize)
skewvar2 <- lm(amount ~ d1*skewness*variance*histavg, data = dhistsize)
summary(skewvar2)
texreg(skewvar)
```
