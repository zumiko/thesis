...
dfc <- dfc %>% dplyr::filter(lagtype != "unknown")
dhist5 <- dhist %>% dplyr::filter(lagtype == "five")
dhist10 <- dhist %>% dplyr::filter(lagtype == "ten")

dhisttest <- dhisttest %>% dplyr::filter(lagtype != "unknown") %>% mutate(amountln = log(amount))
dhist5test <- dhisttest %>% dplyr::filter(lagtype == "five")
dhist10test <- dhisttest %>% dplyr::filter(lagtype == "ten")
...
meanlmcen <- lm(amountcensored ~ histavg*lagtype , data = dhist)
...
```{r}
#var,skew, avg
timeafter<- lm(timeafter ~ histavg , data = dhist5)
timeafter2<- lm(timeafter ~ histavg , data = dhist10)
summary(timeafter)
summary(timeafter2)
```
...
```{r}
#combo 
alllm <- lm(amount ~ histavg + anonprop +  gender, data = dhist)
summary(alllm)
#texreg(alllm)
```
