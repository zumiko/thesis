...
```{r}
asdf <- lm(amount ~ d1 + variance + skewness, data = dhistsize)
```
...
variance <- apply(dhistsize[,23:27], 1, var)
skew <- apply(dhistsize[,23:27], 1, skewness)
...
```{r}
dhistree10 <- na.omit(dhist) %>% dplyr::filter(lagtype == "ten") %>% mutate(amount = unlist(amount))
dhistree <- dhistree10 %>% select(amount, skewness, variance, histavg, is_female, lag1)
